...
public class Exam0521 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // 파일의 데이터를 한 번에 읽어보자.
    byte[] buf = new byte[1000]; // 41 42 b0 a1 b0 a2
    ...
    //  => JVM 환경 변수 'file.encoding' 에 설정된 문자집합으로 가정한다.
    //  => 그래서 그 문자집합이라 가정한 상태에서 해당 바이트를 UCS2으로 변환한다.
    //
    // 1) 이클립스에서 실행하면 => 성공!
    // - JVM 실행 옵션에 '-Dfile.encoding=UTF-8' 환경 변수가 자동으로 붙는다.
    // - 그래서 String 클래스는 바이트 배열이 UCS2로 바꿀 때 UTF-8 문자표를 사용하여 USC2 로 변환한다.
    // - 예1)
    ...
    //   해당 파일을 UTF-8 문자로 간주하여 UTF-8 문자표에 따라 UCS2로 변환된다.
