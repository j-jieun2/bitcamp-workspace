...
public class Exam0210 {
  ...
  // MyThreadPool과 MyThread 상호간에 참조를 피하기 위해
  // 인터페이스를 준비했다.
  ...
  static class MyThreadPool implements ThreadPool {
    ...
    @Override
    public MyThread get() {
      if (list.size() > 0) { // 컬렉션에 남아있는 Thread가 있다면,
        return list.remove(0);
      }
      return null; // 없으면 null을 리턴한
