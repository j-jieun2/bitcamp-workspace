...
public class ProjectUpdateCommand implements Command {
  ...
  public void execute(Request request) {
    ...
    try {
      ...
      String value = Prompt.inputString(String.format("프로젝트명(%s)? ", project.getTitle()), out, in);
      if (value.length() > 0) {
        project.setTitle(value);
      }

      value = Prompt.inputString(String.format("내용(%s)? ", project.getContent()), out, in);
      if (value.length() > 0) {
        project.setContent(value);
      }

      value = Prompt.inputString(String.format("시작일(%s)? ", project.getStartDate()), out, in);
      if (value.length() > 0) {
        project.setStartDate(Date.valueOf(value));
      }

      value = Prompt.inputString(String.format("종료일(%s)? ", project.getEndDate()), out, in);
      if (value.length() > 0) {
