...
public class DefaultProjectService implements ProjectService {
  ...
  public DefaultProjectService(TaskDao taskDao,
      ProjectDao projectDao,
      SqlSessionFactoryProxy factoryProxy) {
  ...
  public int delete(int no) throws Exception {
    try {
      ...
      taskDao.deleteByProjectNo(no); // 프로젝트 작업을 지운다.
      projectDao.deleteMembers(no);
      int count = projectDao.delete(no); // 프로젝트를 지운다.
      factoryProxy.commit(); // 커밋 한다.
      return count; // 지운만큼 카운트해서 리턴한다.
  ...
  public List<Project> list(String keyword) throws Exception {
    ...
    //    어떤 작업을 수행하던지간에 일관되게 Service 객체를 사용하는 것이
    //    유지보수하기에 더 낫다.
    // => 그래서 이런 메서드를 만드는 것이다.
